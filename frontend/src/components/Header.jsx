...
export default function Header({ onLogout }) {
  ...
  // State để điều khiển menu dropdown (thay vì dùng bootstrap js)
  const [showDropdown, setShowDropdown] = useState(false);
  
  const navigate = useNavigate();

  useEffect(() => {
    // API lấy workspace (giữ nguyên logic của bạn)
    fetch("http://localhost:5000/api/v1/workspaces")
      .then((res) => res.json())
      .then((data) => setWorkspaces(data))
      .catch((err) => console.error("Lỗi workspace (có thể do chưa login):", err));
  }, []);
    ...
    >
      ...
      <div className="dropdown">
        ...
        <ul 
            ...
            style={{ 
                // Style này giúp menu hiện lên trên nút bấm thay vì bị che ở dưới
                position: 'absolute', 
